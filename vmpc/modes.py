...
def BUILTIN_MODES() -> tuple[Mode, ...]:  # noqa: N802 - kept as the old constant's name
    """The shipped modes, freshly built on every call.

    A function rather than a module-level tuple: the description is resolved
    through :func:`~vmpc.strings.t` at *this* moment, so a ``/lang`` switch is
    reflected the next time anything asks for the list, with no separate
    "re-translate the built-ins" step to remember. Building three small
    dataclasses is cheap enough that doing it on every ``/mode`` keystroke is
    not worth avoiding.
    """
    return (
        Mode(
...
def validate_name(name: str, taken: "frozenset[str] | set[str]" = frozenset()) -> str:
    ...
    if not cleaned:
        return t("mode.err.empty_name")
    if not _NAME.match(cleaned):
        return t("mode.err.bad_name")
    if cleaned in taken:
        return t("mode.err.taken", name=cleaned)
    return ""
