...
def collect(target: Any, budget: int = DEFAULT_BUDGET) -> Bundle:
    ...
    if not given:
        ...
        # cannot happen.
        raise ContextError("no path given")
    root = Path(given).expanduser()
    try:
        root = root.resolve()
    except OSError as exc:  # pragma: no cover - platform-specific
        raise ContextError(f"cannot resolve {target}: {exc}") from exc
    if not root.exists():
        raise ContextError(f"{root} does not exist")
    if budget <= 0:
        raise ContextError("the context budget is already full")
    ...
    if not root.is_dir():  # pragma: no cover - sockets, devices
        raise ContextError(f"{root} is not a file or a folder")
    ...
    for path in _walk(root):
        ...
        if len(bundle.files) >= MAX_FILES:
            bundle.skipped.append((relative, "over the file limit"))
            continue
        if path.suffix.lower() in SKIP_SUFFIXES:
            bundle.skipped.append((relative, "binary"))
            continue
        if _looks_secret(path.name):
            bundle.skipped.append((relative, "looks like a secret"))
            continue
        if remaining < 400:
            ...
            bundle.skipped.append((relative, "budget"))
            continue
