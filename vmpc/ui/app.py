...
from vmpc.strings import current_language, en_plural, ru_plural, set_ui_language, t
...
class App:
    def __init__(self, config: Config, console: Optional[Console] = None) -> None:
        ...
        # vmpc's own interface language, persisted the same way the mode is
        # below — applied before anything is printed, so even the first line
        # this run ever shows is in the right language.
    ...
    def attach_context(self, announce_errors: bool = True) -> None:
        ...
        try:
            ...
            self.console.print(Text(t("app.context_not_read", exc=exc), style="error"))
            return
        if announce_errors:
            for problem in errors:
                self.console.print(Text(t("app.context_error", problem=problem), style="error"))
        files = sum(len(bundle.files) for bundle in bundles)
        if files:
            file_noun = (
                en_plural(files, "file")
                if current_language() == "en"
                else ru_plural(files, "файл", "файла", "файлов")
            )
            path_noun = (
                en_plural(len(bundles), "path")
                if current_language() == "en"
                else ru_plural(len(bundles), "путь", "пути", "путей")
            )
            self.console.print(
                Text(t("app.context_attached"), style="hint")
                .append(t("app.context_files", n=files, noun=file_noun), style="primary")
                .append(
                    t("app.context_from_paths", n=len(bundles), noun=path_noun),
                    style="secondary",
                )
            )
    ...
    def run(self) -> int:
        ...
        while True:
            ...
            try:
                ...
                self.console.print(Text(t("app.input_error", exc=exc), style="error"))
                return 1

            if line is None:
                self.console.print(Text(t("app.bye"), style="secondary"))
                return 0
            line = line.strip()
            if not line:
                continue
    ...
    def _dispatch(self, line: str) -> bool:
        ...
        if parsed is None:
            ...
            if locked is not None:
                self.console.print(
                    Text(t("app.command_needs", display=locked.display), style="secondary").append(
                        f"/{locked.group}", style="hint"
                    )
                )
                return True
            self.console.print(Text(t("app.unknown_command", name=name), style="error"))
            self.console.print(Text(t("app.help_hint"), style="secondary"))
            return True

        command, args = parsed
        if command.handler is None:
            self.console.print(
                Text(t("app.not_implemented", display=command.display), style="secondary")
            )
            return True
    ...
    def _turn(self, message: str) -> None:
        ...
        if provider is None:
            self.console.print(Text(t("app.no_endpoint"), style="error"))
            self.console.print(Text(t("app.api_add_hint"), style="hint"))
            return
        problems = provider.validate()
        if problems:
            self.console.print(
                Text(t("app.provider_problem", name=provider.name, problem=problems[0]), style="error")
            )
            self.console.print(Text(t("app.api_edit_hint"), style="hint"))
            return
        ...
        if result.interrupted:
            self.console.print(Text(t("app.interrupted"), style="secondary"))
            if result.text:
        ...
        if result.text:
            ...
            self.console.print(Text(t("app.empty_response"), style="secondary"))
            self.session.drop_last_user()
    ...
    def _persist(self) -> None:
        ...
        try:
            ...
            if not getattr(self, "_save_warned", False):
                ...
                self.console.print(Text(t("app.chat_not_saved", exc=exc), style="error"))
    ...
    def _report_error(self, error: ApiError) -> None:
        self.console.print(Text(t("app.error_prefix", error=error), style="error"))
        if error.hint:
            self.console.print(Text(t("app.error_hint", hint=error.hint), style="secondary"))
    ...
    def _print_banner(self) -> None:
        ...
        if provider is not None:
            ...
            row = Text("  │  ", style="secondary")
            row.append(t("app.banner_no_endpoint"), style="secondary")
        ...
        if paths:
            ...
            path_label = (
                ...
                if len(paths) == 1
                    ...
                    n=len(paths),
                    noun=(
                        en_plural(len(paths), "path")
                        if current_language() == "en"
                        else ru_plural(len(paths), "путь", "пути", "путей")
                    ),
                )
            )
            row.append(path_label, style="header")
            row.append(t("app.banner_attached"), style="secondary")
            row.truncate(max(self.console.width - 1, 20), overflow="ellipsis")
            self.console.print(row)

        footer = Text("  ╰─ ", style="secondary")
        footer.append("/help", style="hint")
        footer.append(t("app.banner_for_commands"), style="secondary")
    ...
    def _placeholder(self) -> str:
        provider = self.config.active_provider()
        if provider is None:
            return t("app.placeholder_no_endpoint")
        return t("app.placeholder_ready")
