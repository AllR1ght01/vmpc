...
class StatusIndicator:
    ...
    #: Ctrl+C, not Esc: during a turn the composer is not reading keys, so a
    #: bare Esc reaches nobody. The guard routes SIGINT to the cancel event.
    interrupt_hint: str = "Ctrl+C to interrupt"
    tokens: int = 0
    truecolor: bool = True
    ...
    def __rich_console__(
        ...
        if self.tokens:
            line.append(f"  {_compact_tokens(self.tokens)} tok", style="secondary")
