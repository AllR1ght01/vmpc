...
def REASONING_HEADER() -> str:  # noqa: N802 - kept as the old constant's name
    """Emitted once above a reasoning section.

    A function, not a module-level string: see ``BUILTIN_MODES()`` in
    :mod:`vmpc.modes` for why — resolved through :func:`vmpc.strings.t` at
    call time so ``/lang`` is reflected on the very next turn, not just after
    a restart.
    """
