...
def WIRE_DESCRIPTIONS() -> dict[str, str]:  # noqa: N802 - kept as the old constant's name
    """A function, not a module-level dict: see ``BUILTIN_MODES()`` in
    :mod:`vmpc.modes` for why — resolved through :func:`vmpc.strings.t` at call
    time so ``/lang`` is reflected the next time the wizard opens."""
    from vmpc.strings import t

    return {
        WIRE_OPENAI: t("wire_desc.openai"),
        WIRE_ANTHROPIC: t("wire_desc.anthropic"),
    }
...
def KEY_TYPE_DESCRIPTIONS() -> dict[str, str]:  # noqa: N802
    from vmpc.strings import t

    return {
        KEY_TYPE_STATIC: t("keytype_desc.static"),
        KEY_TYPE_ENV: t("keytype_desc.env"),
        KEY_TYPE_COMMAND: t("keytype_desc.command"),
    }
...
def AUTH_DESCRIPTIONS() -> dict[str, str]:  # noqa: N802
    from vmpc.strings import t

    return {
        AUTH_BEARER: "Authorization: Bearer <key>",
        AUTH_X_API_KEY: "x-api-key: <key>",
        AUTH_RAW: "Authorization: <key>  (" + t("auth_desc.raw_suffix") + ")",
        AUTH_CUSTOM: "<your-header>: <key>",
        AUTH_NONE: t("auth_desc.none"),
    }
