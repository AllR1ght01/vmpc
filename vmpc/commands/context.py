...
def _attach(console: Any, session: Any, target: str) -> None:
    ...
    if not cleaned:
        console.print(Text(t("ctx.which_folder"), style="secondary"))
        return
    try:
    ...
    if stored in session.context_paths:
        console.print(Text(t("ctx.already_attached", path=stored), style="secondary"))
    else:
        session.context_paths.append(stored)

    bundles, errors = context_module.apply(session)
    _report(console, session, bundles, errors, header=t("ctx.header_context"))


def _detach(console: Any, session: Any, target: str) -> None:
    cleaned = target.strip().strip(_QUOTES).strip()
    if not cleaned:
        console.print(Text(t("ctx.which_one"), style="secondary"))
        return
    ...
    if len(keep) == len(session.context_paths):
        console.print(Text(t("ctx.not_attached", path=cleaned), style="secondary"))
        return
...
def _clear(console: Any, session: Any) -> None:
    if not session.context_paths:
        console.print(Text(t("ctx.nothing_attached"), style="secondary"))
        return
    count = len(session.context_paths)
    session.context_paths.clear()
    session.context_text = ""
    noun = (
        en_plural(count, "path")
        if current_language() == "en"
        else ru_plural(count, "путь", "пути", "путей")
    )
    console.print(
        Text(t("ctx.detached"), style="secondary").append(
            f"{count} {noun}", style="primary"
        )
    )
    ...
    if not session.context_paths:
        ...
        console.print(
            Text("  /context ", style="hint").append(
                t("ctx.hint_path"), style="secondary"
            )
        )
        return
    bundles, errors = context_module.apply(session)
...
def _report(
    ...
    for bundle in bundles:
        ...
        file_noun = (
            en_plural(len(bundle.files), "file")
            if current_language() == "en"
            else ru_plural(len(bundle.files), "файл", "файла", "файлов")
        )
        row.append(
            f"  {len(bundle.files)} {file_noun}",
            style="secondary",
        )
        ...
        if reasons:
            ...
            console.print(
                Text(t("ctx.skipped_label"), style="secondary").append(detail, style="secondary")
            )
        if bundle.budget_hit:
            console.print(
                Text("  │   ", style="secondary").append(
                    t("ctx.budget_ran_out"), style="error"
                )
            )
    ...
    if total:
        ...
        footer.append(t("ctx.tokens_added"), style="secondary")
    footer.append(t("ctx.clear_hint"), style="hint")
    footer.append(t("ctx.to_detach"), style="secondary")
