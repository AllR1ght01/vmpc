...
def _pick(console: Console, config: Config, provider: Provider) -> bool:
    ...
    rows.append(Choice(_ACTION_ADD, "+ type a name", description="add it by hand"))
    rows.append(
        Choice(
            _ACTION_PROBE,
            "⌕ probe",
            description="try known names against this key",
        )
    )
    if not live:
        rows.append(
            Choice(
                _ACTION_SCAN,
                "↻ ask the endpoint",
                description="retry GET /models",
            )
        )

    chosen = select(f"Model for {provider.name}", rows)
    if chosen is None:
        return False
...
def _use(console: Console, config: Config, provider: Provider, name: str) -> bool:
    ...
    config.save()
    console.print(
        Text("  model ", style="secondary").append(provider.model, style="success")
    )
    return True
...
def _add(console: Console, config: Config, provider: Provider, name: str) -> bool:
    ...
    if not entered:
        entered = prompt_text(
            "model:",
            placeholder="the exact name the endpoint expects",
        ) or ""
    ...
    if entered in saved:
        console.print(Text(f"  already saved: {entered}", style="secondary"))
    else:
    ...
    config.save()
    console.print(
        Text("  model ", style="secondary")
        .append(entered, style="success")
        .append("  saved on ", style="secondary")
...
def _remove(console: Console, config: Config, provider: Provider, name: str) -> bool:
    wanted = name.strip()
    if not wanted:
        console.print(Text("  which one? /model rm NAME", style="secondary"))
        return False
    saved = list(getattr(provider, "models", []) or [])
    if wanted not in saved:
        console.print(Text(f"  not saved: {wanted}", style="secondary"))
        return False
    provider.models = [item for item in saved if item != wanted]
    config.upsert(provider)
    config.save()
    console.print(Text("  forgot ", style="secondary").append(wanted, style="primary"))
    if wanted == provider.model:
        ...
        console.print(
            Text("  still using it — ", style="secondary").append(
                "/model NAME", style="hint"
            ).append(" to switch", style="secondary")
        )
    return True
...
def _print_list(console: Console, provider: Provider) -> None:
    ...
    if not saved and not live:
        console.print(Text("  nothing saved, and the endpoint did not answer", style="secondary"))
        console.print(
    ...
    for name in known_models(provider, live):
        ...
        where = "saved" if name in saved else ("advertised" if name in live else "")
        console.print(
...
def _scan(console: Console, config: Config, provider: Provider) -> bool:
    ...
    if not live:
        ...
        console.print(Text("  /model probe — find out by asking", style="hint"))
        return False
    saved = list(getattr(provider, "models", []) or [])
    ...
    console.print(
        ...
        .append(" advertised · ", style="secondary")
        .append(str(len(added)), style="success")
        .append(" new", style="secondary")
    )
    return True
...
def _probe(console: Console, config: Config, provider: Provider, rest: str) -> bool:
    ...
    console.print()
    console.print(
        Text("  probe ", style="header")
        .append(str(len(names)), style="primary")
        .append(" names against ", style="secondary")
        .append(provider.endpoint(), style="hint")
    )
    console.print(
        Text("  ", style="secondary").append(
            f"{len(names)} requests, one token each · esc to stop", style="secondary"
        )
    )
    answer = confirm("Send them?", default=True)
    if not answer:
    ...
    try:
        for result in probe_module.probe(provider, names, cancel=cancel):
            ...
            if result.available:
                ...
                console.print(
                    Text("    stopping — that answer is about the key, not the name",
                          style="secondary")
                )
                ...
                if errors >= 5:
                    ...
                    cancel.set()
                    console.print(
                        Text("    too many failures — stopping", style="error")
                    )
    except KeyboardInterrupt:
        cancel.set()
        console.print(Text("  interrupted", style="secondary"))
    ...
    config.save()
    console.print(
        Text("  saved ", style="secondary")
        .append(str(len(added)), style="success")
        .append(" · using ", style="secondary")
...
def _probe_summary(console: Console, counts: dict[str, int], asked: int) -> None:
    ...
    labels = (
        (probe_module.KIND_AVAILABLE, "available"),
        (probe_module.KIND_REJECTED, "rejected"),
        (probe_module.KIND_UNCLEAR, "unclear"),
        (probe_module.KIND_ERROR, "failed"),
        (probe_module.KIND_SKIPPED, "not sent"),
    )
    row = Text("  ", style="secondary")
    row.append(f"{asked} asked", style="secondary")
...
def _wanted_names(console: Console, rest: str) -> Optional[list[str]]:
    ...
    if candidate.is_file():
        try:
            ...
            console.print(Text(f"  ✖ cannot read {candidate}: {exc}", style="error"))
            return None
        return [
            line.strip()
            for line in lines
...
def _list_models(provider: Provider) -> Optional[list[str]]:
    ...
    try:
        from vmpc.api.client import _headers
