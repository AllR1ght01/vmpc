...
def show_raw(context: CommandContext, args: str) -> None:
    ...
    if len(matching) < back:
        console.print(Text(f"  no {role} message that far back", style="secondary"))
        return
    ...
    console.print(
        Text(f"  {role}", style="header").append(
            f"  {len(body)} chars, {lines} line{'' if lines == 1 else 's'}",
            style="secondary",
        )
    )
...
def render_sample(context: CommandContext, args: str) -> None:
    ...
    if source.lower() == "last":
        ...
        if not replies:
            console.print(Text("  no reply to re-render", style="secondary"))
            return
    ...
    console.print()
    console.print(
        Text("  render ", style="header").append(
            f"{label} · width {max(console.width - 2, 20)}", style="secondary"
        )
    )
    console.print()
...
def show_wire(context: CommandContext, args: str) -> None:
    ...
    console.print()
    console.print(
        Text("  POST ", style="header").append(provider.endpoint(), style="primary")
    )
    transport = "vendor SDK" if provider.use_sdk else "httpx"
    console.print(
        Text("  via  ", style="secondary").append(transport, style="hint")
    )
    console.print()

    if header_error:
        console.print(Text(f"  ✖ credential: {header_error}", style="error"))
...
def ping_endpoint(context: CommandContext, args: str) -> None:
    ...
    console.print()
    console.print(
        Text("  ping ", style="header").append(probe.endpoint(), style="secondary")
    )
    ...
    rows = [("total", f"{total * 1000:.0f} ms")]
    if first is not None:
        rows.append(("first token", f"{(first - started) * 1000:.0f} ms"))
        streaming = total - (first - started)
        if output_tokens and streaming > 0.001:
            rows.append(("throughput", f"{output_tokens / streaming:.1f} tok/s"))
    rows.append(("output", f"{output_tokens or '?'} tokens, {chars} chars"))
    ...
    if first is None:
        console.print(Text("  ✖ no content came back", style="error"))
    console.print()
...
def show_palette(context: CommandContext, args: str) -> None:
    ...
    facts = [
        ("color system", str(console.color_system)),
        ("no_color", str(console.no_color)),
        ("terminal", str(console.is_terminal)),
        ("legacy windows", str(console.legacy_windows)),
        ("width", str(console.width)),
    ]
    console.print()
...
def show_prompt(context: CommandContext, args: str) -> None:
    ...
    console.print()
    console.print(
        Text("  system prompt", style="header").append(
            f"  {len(system)} chars", style="secondary"
        )
    )
    console.print(
        Text("  mode  ", style="secondary").append(
            f"/{mode}" if mode else "(none — default prompt only)",
            style="hint" if mode else "secondary",
        )
    )
    if attached:
        console.print(
            Text("  files ", style="secondary").append(
