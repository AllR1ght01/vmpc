...
def _show_modes(context: CommandContext) -> None:
    ...
    for mode in modes:
        ...
        if mode.name not in builtin:
            row.append("  custom", style="secondary")
    ...
    console.print(
        Text("  /mode add", style="hint").append(
            "  ·  /mode edit NAME  ·  /mode remove NAME  ·  /mode off",
            style="secondary",
        )
    )
    console.print()
...
def _show_one(context: CommandContext, name: str) -> None:
    ...
    if mode is None:
        console.print(Text(f"  no mode named {name!r}", style="error"))
        return
    console.print()
    console.print(
    ...
    if mode.unlocks:
        console.print(
            Text("  unlocks  ", style="secondary").append(
                ", ".join(mode.unlocks), style="hint"
            )
        )
    console.print()
    for line in (mode.prompt or "(no prompt — the default one is used as-is)").splitlines():
...
def activate_mode(context: CommandContext, name: str) -> bool:
    ...
    if mode is None:
        console.print(Text(f"  no mode named {name!r}", style="error"))
        console.print(
            Text("  /mode", style="hint").append(" — list them", style="secondary")
        )
        return False
    ...
    if mode.unlocks:
        ...
        console.print(
            Text("  new commands unlocked — ", style="secondary").append(
                "/help", style="hint"
            )
        )
    console.print()
...
def _clear_mode(context: CommandContext) -> bool:
    ...
    if not getattr(config, "mode", ""):
        console.print(Text("  already on the default prompt", style="secondary"))
        return False
    config.mode = ""  # type: ignore[union-attr]
    config.save()  # type: ignore[union-attr]
    _apply_to_session(context, None)
    console.print(
        Text("  mode off", style="secondary").append(
            " — back to the default prompt", style="secondary"
        )
    )
    return True
...
def _add_mode(context: CommandContext, name: str) -> bool:
    ...
    if name:
        ...
        name = prompt_text("name", placeholder="reviewer", validate=check) or ""
    name = name.strip().lower()
    if not name:
        return False

    description = prompt_text("description", placeholder="what this mode is for")
    if description is None:
        return False

    prompt = prompt_multiline("prompt", placeholder="how the model should answer")
    if not prompt:
        console.print(
            Text("  cancelled — a mode with no prompt would do nothing", style="secondary")
        )
        return False

    unlock = confirm("Unlock the developer commands in this mode?", default=False)
    if unlock is None:
        return False
    ...
    context.config.modes.append(  # type: ignore[union-attr]
        Mode(
            name=name,
            description=description or "custom mode",
            prompt=prompt,
    ...
    console.print(
        ...
        .append(" saved — type it to switch", style="secondary")
    )
    return True
...
def _edit_mode(context: CommandContext, name: str) -> bool:
    ...
    if match is None:
        if find_mode(wanted, ()) is not None:
            ...
            console.print(
                Text(f"  /{wanted} is built in — ", style="error")
                .append("/mode add", style="hint")
                .append(" to write your own", style="secondary")
            )
        else:
            console.print(Text(f"  no custom mode named {name!r}", style="error"))
        return False

    description = prompt_text("description", default=match.description)
    if description is None:
        return False
    prompt = prompt_multiline("prompt", default=match.prompt)
    if not prompt:
        console.print(Text("  cancelled — empty prompt", style="secondary"))
        return False
    unlock = confirm(
        "Unlock the developer commands in this mode?",
    ...
    console.print(Text(f"  ✔ /{match.name} updated", style="success"))
    return True
...
def _remove_mode(context: CommandContext, name: str) -> bool:
    ...
    if getattr(config, "mode", "") == match.name:
        ...
        _apply_to_session(context, None)
    config.save()  # type: ignore[union-attr]
    console.print(Text(f"  removed /{match.name}", style="secondary"))
    return True
