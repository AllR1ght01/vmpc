...
class Session:
    ...
    def full_system(self) -> str:
        ...
        Note this has nothing to do with ``/lang``: that switches vmpc's own
        interface text (see :mod:`vmpc.strings`), not the model's reply
        language, which the base system prompt above already leaves to the
        model to match to the user.
        """
