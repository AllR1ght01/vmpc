...
def probe(
    ...
    # Managed by hand rather than `with` blocks: when the caller abandons the
    # generator mid-sweep (Ctrl+C in the /model probe loop), `with` would run
    # pool.shutdown(wait=True) and block until every queued probe had run to
    # its 20s timeout. Cancelling first makes an interrupt take effect now.
    client = httpx.Client(timeout=PROBE_TIMEOUT, follow_redirects=True)
